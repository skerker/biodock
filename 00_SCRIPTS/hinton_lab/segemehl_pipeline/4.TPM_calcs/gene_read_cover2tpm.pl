...
-format           simple (only gene ID and TPM value)
-help             this help text
...
if ($gene_length_file
    or
    $length_in_data_file) {
    push @header, 'gene length' unless ($format eq 'simple');
}
foreach (@methods) {
    my %done = ();
    if (defined $in{$_}) {
	...
	push @header, "min $_\tmax $_\tmean $_" unless ($format eq 'simple');
    }
}
...
foreach my $gene (sort keys %{$in{'raw'}}) {
    ...
    if (%lengths) {
	push @out, $lengths{$gene} unless ($format eq 'simple');
    }
